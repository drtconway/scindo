    ...
    namespace vcf
    {
        ...
        struct vcf_file_reader
        {
                  ...
                  header(NULL), var(NULL)
            {
                if (!file)
                {
                    auto msg = std::string("unable to open VCF file `") + filename + std::string("` for reading");
                ...
                if (!header)
                {
                    auto msg = std::string("unable read header for VCF file `") + filename + std::string("`");
                    throw std::runtime_error(msg);
                }
            }
            ...
            bool next()
            {
                if (!var)
                {
                    ...
                    if (!var)
                    {
                        auto msg = std::string("unable to allocate variant for VCF file `") + filename + std::string("`");
                        throw std::runtime_error(msg);
                    }
                }
                ...
                if (res < -1)
                {
                    auto msg = std::string("error reading VCF file `") + filename + std::string("`");
                    throw std::runtime_error(msg);
                }
                return (res == 0);
            }
            ...
            template<typename T>
            std::enable_if<std::is_integral<T>::value, bool>::type info_value(size_t p_n, T& p_res)
            {
                bcf_unpack(var, BCF_UN_INFO);
                const bcf_info_t& ifo = var->d.info[p_n];
                p_res = ifo.v1.i;
            ...
            template<typename T>
            std::enable_if<std::is_floating_point<T>::value, bool>::type info_value(size_t p_n, T& p_res)
            {
                bcf_unpack(var, BCF_UN_INFO);
                const bcf_info_t& ifo = var->d.info[p_n];
                p_res = ifo.v1.f;
            ...
            template<typename T>
            std::enable_if<std::is_integral<T>::value, bool>::type info_value(const info_id& p_id, T& p_res)
            {
                bcf_unpack(var, BCF_UN_INFO);
            ...
            template<typename T>
            std::enable_if<std::is_floating_point<T>::value, bool>::type info_value(const info_id& p_id, T& p_res)
            {
                bcf_unpack(var, BCF_UN_INFO);
                const bcf_info_t* ptr = bcf_get_info_id(var, p_id.id);
                if (!ptr)
            ...
            template <typename T>
            std::optional<T> info_value(const info_id& p_id)
            {
                T x;
            ...
            template <typename T>
            struct format_type_tag<T, typename std::enable_if<std::is_same<T,int32_t>::value, void>::type>
            {
            ...
            template <typename T>
            struct format_type_tag<T, typename std::enable_if<std::is_same<T,int64_t>::value, void>::type>
            {
            ...
            template <typename T>
            struct format_type_tag<T, typename std::enable_if<std::is_same<T,float>::value, void>::type>
            {
            ...
            template <typename T>
            struct format_type_tag<T, typename std::enable_if<std::is_same<T,std::string>::value, void>::type>
            {
            ...
            struct format_data
            {
                ...
                const T* data() const
                {
                    return reinterpret_cast<const T*>(arr_ptr);
                }
            ...
            {
                int r0 = bcf_get_format_values(header, var, p_tag,
                                               &(p_res.arr_ptr), &(p_res.arr_size),
                                               format_type_tag<T>::type_tag);
                return r0 >= 0;
            }
